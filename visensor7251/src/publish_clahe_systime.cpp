...
ros::Time first_img_timesys;
ros::Time first_img_timefpga;
...
void imu_process(ros::NodeHandle *nh ,device *cam)
{
  ...
  while(nh->ok()) {
    if(cam->readIMU(&imudata)){
      ...
      // 把时间戳转换成系统时间
      if(first_img_timesys.toSec() > 0 && first_img_timefpga.toSec() > 0)
	timestamp = first_img_timesys +
	  ros::Duration(timestamp.toSec() - first_img_timefpga.toSec());
...
int main(int argc, char** argv)
{
  ...
  while (nh.ok()) {
    if (cam->QueryFrame(left8u, right8u, imgTimeStamp)) {
      ...
      // 记录第一帧图像的时间
      if(0 == aa){
	first_img_timesys = ros::Time::now();
	first_img_timefpga = timestamp;
      }
	...
	ros::Duration(timestamp.toSec() - first_img_timefpga.toSec());
      ...
      cvi0.encoding = "mono8";
      clahe->apply(left8u, left_equalize);
      cvi0.image = left_equalize;
      
      cvi1.header.stamp =  cvi0.header.stamp;
      cvi1.header.frame_id = "image1";
      cvi1.encoding = "mono8";
      clahe->apply(right8u, right_equalize);
      cvi1.image = right_equalize;
