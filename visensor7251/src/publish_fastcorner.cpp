...
int main(int argc, char** argv)
{
  ...
  Mat color = Mat(Size(rn.width, rn.height), CV_8UC3);
  cv::Ptr<cv::Feature2D> detector_ptr = FastFeatureDetector::create(10,true,FastFeatureDetector::TYPE_9_16);
  ...
  while (nh.ok()) {
	...
	continue;
      ...
      detector_ptr->detect(left8u, new_features);
      cvtColor(left8u, color, CV_GRAY2RGB);
      for(int i=0;i<new_features.size();i++) {
	circle(color, new_features[i].pt, 3, Scalar(0,0,255), -1); //x
      }
      vector<Point2f> gfft_features(0);
      cv::goodFeaturesToTrack(left8u, gfft_features, 200, 0.001, 10);
